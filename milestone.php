    ...
    public function action_rowfunction()
    {
        ...
        if (Input::method() == 'POST') {
            ...
            foreach ($colors as $color) {
                $color_selects[] = $color;
                $currId = 'color_' . str_replace(' ', '_', strtolower($color['name']));
                $color_table .= '<tr>';
                $color_table .= '<td style="border: 1px solid black; width: 20%;"><select class="color-dropdown" data-index="' . $currId . '">';
                foreach ($colors as $c) {
            ...
            for ($i = 1; $i <= $row_value + 1; $i++) {
                ...
                for ($j = 1; $j <= $row_value + 1; $j++) {
                    if ($i == 1 && $j == 1) {
                        ...
                        $normal_table .= '<td id="cell' . $i . ',' . $j . '" class="myCell">' . ($i - 1) . ',' . ($j - 1) . '</td>';
                    }
                }
                $normal_table .= '</tr>';
            }
